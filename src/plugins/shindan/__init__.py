...
def shindan_handler(shindan: ShindanConfig) -> T_Handler:
    async def handler(
        ...
        if not name and uninfo and uninfo.user:
            name = uninfo.user.nick or uninfo.user.name

        if not name and hasattr(event, "sender") and getattr(event.sender, "nickname", None):
...
def refresh_matchers():
    ...
    for shindan in shindan_manager.shindan_list:
        matcher = on_alconna(
            ...
            block=True,
            priority=14,
            use_cmd_start=False,  # 无需前缀，直接识别「魔法人生」等
        )
...
@driver.on_startup
async def _():
    from .manager import init_default_shindan_if_empty
    await init_default_shindan_if_empty()

...
# 输入框与按钮的多种选择器（站点改版时可能变化）
INPUT_SELECTORS = [
    'input#user_input_value_1',
    'input[id="user_input_value_1"]',
    '#shindanForm input[type="text"]',
    'form input[type="text"]',
]
SUBMIT_SELECTORS = [
    'button#shindanButtonSubmit',
    'button[id="shindanButtonSubmit"]',
    '#shindanForm button[type="submit"]',
    '#shindanForm input[type="submit"]',
    ...
    async with get_new_page() as page:
        ...
        for sel in INPUT_SELECTORS:
            ...
            try:
                ...
                continue
        if not input_loc:
            raise RuntimeError("无法找到占卜输入框，站点结构可能已变更")
        await input_loc.fill(name + seed, timeout=10000)
        btn_loc = None
        for sel in SUBMIT_SELECTORS:
            ...
            try:
                ...
                continue
        if not btn_loc:
            raise RuntimeError("无法找到占卜提交按钮，站点结构可能已变更")
        await btn_loc.click()

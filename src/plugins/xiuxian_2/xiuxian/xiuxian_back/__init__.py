...
batch_gf_re_root = on_command("批量炼金功法", priority=6, permission=GROUP, block=True)
batch_st_re_root = on_command("批量炼金神通", priority=6, permission=GROUP, block=True)
...
mix_tianzui = on_command("合成天罪", priority=6, permission=GROUP, block=True)
mix_dijue = on_command("合成帝绝魔铠", priority=6, permission=GROUP, block=True)
...
12、批量炼金功法:将背包内全部功法炼化为灵石（不影响已学习的功法）
13、批量炼金神通:将背包内全部神通炼化为灵石（不影响已学习的神通）
14、背包帮助:获取背包帮助指令
15、查看修仙界物品:支持类型【功法|神通|丹药|合成丹药|法器|防具】
16、清空坊市:清空本群坊市,管理员权限
17、合成天罪:拥有 无罪（残缺） 与 原罪（残缺）时合成天罪
18、合成帝绝魔铠:消耗 东皇钟 与 血海魔铠，15%概率合成成功（失败也会消耗）
非指令：
...
@batch_gf_re_root.handle(parameterless=[Cooldown(at_sender=True)])
async def batch_gf_re_root_(bot: Bot, event: GroupMessageEvent):
    """批量炼金功法：将背包内全部功法炼化为灵石"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    isUser, user_info, msg = check_user(event)
    if not isUser:
        if XiuConfig().img:
    ...
    for back in back_msg:
        if back.goods_type != "技能":
            continue
        goods_id = back.goods_id
        goods_num = back.goods_num
        item_info = items.get_data_by_item_id(goods_id)
        if item_info.get("item_type") != "功法":
            continue

        rank = get_item_msg_rank(goods_id)
        if rank == 520:
            continue
        price_single = int(6000000 - rank * 100000)
        if price_single <= 0:
            continue
        price = price_single * int(goods_num)
        if price <= 0:
            continue

        sql_message.update_back_j(user_id, goods_id, num=int(goods_num))
        sql_message.update_ls(user_id, price, 1)
        total_price += price
        total_num += int(goods_num)
        detail_list.append(f"{item_info['level']}功法-{item_info['name']} x{goods_num} -> {price} 灵石")

    if total_price <= 0:
        msg = "道友背包内没有可以炼金的功法！"
    else:
        detail = "\n".join(detail_list[:50])
        msg = f"本次共炼金功法数量：{total_num}，共获得灵石：{total_price}枚！"
        if detail:
            msg += f"\n（部分明细）\n{detail}"
...
@batch_st_re_root.handle(parameterless=[Cooldown(at_sender=True)])
async def batch_st_re_root_(bot: Bot, event: GroupMessageEvent):
    """批量炼金神通：将背包内全部神通炼化为灵石"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    isUser, user_info, msg = check_user(event)
    if not isUser:
        if XiuConfig().img:
    ...
    for back in back_msg:
        ...
        total_price += price
        total_num += int(goods_num)
        detail_list.append(f"{item_info['level']}神通-{item_info['name']} x{goods_num} -> {price} 灵石")

    if total_price <= 0:
        msg = "道友背包内没有可以炼金的神通！"
    else:
        detail = "\n".join(detail_list[:50])
        msg = f"本次共炼金神通数量：{total_num}，共获得灵石：{total_price}枚！"
        if detail:
            msg += f"\n（部分明细）\n{detail}"
...
@mix_tianzui.handle(parameterless=[Cooldown(at_sender=True)])
async def mix_tianzui_(bot: Bot, event: GroupMessageEvent):
	bot, send_group_id = await assign_bot(bot=bot, event=event)
	isUser, user_info, msg = check_user(event)
	if not isUser:
		if XiuConfig().img:
			...
			await bot.send_group_msg(group_id=int(send_group_id), message=msg)
		await mix_tianzui.finish()

	user_id = user_info.user_id
	yuan_zui_id = 7111  # 原罪（残缺）
	wu_zui_id = 7112  # 无罪（残缺）
	tian_zui_id = 7101  # 天罪

	yz = sql_message.get_item_by_good_id_and_user_id(user_id, yuan_zui_id)
	wz = sql_message.get_item_by_good_id_and_user_id(user_id, wu_zui_id)
	if not yz or not wz or yz.goods_num < 1 or wz.goods_num < 1:
		msg = "合成天罪需要：原罪（残缺）×1 + 无罪（残缺）×1（都在背包且各≥1）"
		if XiuConfig().img:
			pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
			await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
		else:
	...
	# 若材料被装备，要求先卸载，避免把已装备的物品直接扣掉
	if int(yz.state) == 1 or int(wz.state) == 1:
		msg = "请先卸载已装备的 原罪（残缺）/无罪（残缺），再进行合成（指令：换装 物品名）"
		if XiuConfig().img:
			pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
			await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
		else:
	...
	# 扣除材料
	sql_message.update_back_j(user_id, yuan_zui_id, num=1)
	sql_message.update_back_j(user_id, wu_zui_id, num=1)

	# 发放天罪
	tz_info = items.get_data_by_item_id(tian_zui_id)
	sql_message.send_back(user_id, tian_zui_id, tz_info["name"], "装备", 1)

	msg = f"合成成功：{tz_info['name']}×1（已消耗：原罪（残缺）×1、无罪（残缺）×1）"
	if XiuConfig().img:
		pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
		await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
	else:
...
@mix_dijue.handle(parameterless=[Cooldown(at_sender=True)])
async def mix_dijue_(bot: Bot, event: GroupMessageEvent):
	bot, send_group_id = await assign_bot(bot=bot, event=event)
	isUser, user_info, msg = check_user(event)
	if not isUser:
		if XiuConfig().img:
			...
			await bot.send_group_msg(group_id=int(send_group_id), message=msg)
		await mix_dijue.finish()

	user_id = user_info.user_id
	donghuang_id = 7099  # 东皇钟（法器）
	xuehai_id = 6108  # 血海魔铠（防具）
	dijue_id = 6109  # 帝绝魔铠（防具）

	dh = sql_message.get_item_by_good_id_and_user_id(user_id, donghuang_id)
	xh = sql_message.get_item_by_good_id_and_user_id(user_id, xuehai_id)
	if not dh or not xh or dh.goods_num < 1 or xh.goods_num < 1:
		msg = "合成帝绝魔铠需要：东皇钟×1 + 血海魔铠×1（都在背包且各≥1）"
		if XiuConfig().img:
			pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
			await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
		else:
	...
	# 若材料被装备，要求先卸载，避免把已装备的物品直接扣掉
	if int(dh.state) == 1 or int(xh.state) == 1:
		msg = "请先卸载已装备的 东皇钟/血海魔铠，再进行合成（指令：换装 物品名）"
		if XiuConfig().img:
			pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
			await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
		else:
	...
	# 无论成功失败，都消耗材料
	sql_message.update_back_j(user_id, donghuang_id, num=1)
	sql_message.update_back_j(user_id, xuehai_id, num=1)

	success = random.random() < 0.15
	if success:
		dj_info = items.get_data_by_item_id(dijue_id)
		sql_message.send_back(user_id, dijue_id, dj_info["name"], "装备", 1)
		msg = f"合成成功：{dj_info['name']}×1（消耗：东皇钟×1、血海魔铠×1）"
	else:
		msg = "合成失败：东皇钟×1、血海魔铠×1 已消失"

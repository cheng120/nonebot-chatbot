...
def get_weapon_info_msg(weapon_id, weapon_info=None):
    """
    获取一个法器(武器)信息msg，有 desc 时优先使用 desc 中的效果说明
    """
    msg = ''
    ...
    if weapon_info.get('desc'):
        for line in weapon_info['desc'].strip().split('\n'):
            if line.startswith('效果：'):
                msg += line.rstrip() + '\n'
                return msg
        msg += weapon_info['desc'].strip() + '\n'
    else:
        atk_buff = weapon_info.get('atk_buff', 0) or 0
        crit_buff = weapon_info.get('crit_buff', 0) or 0
        atk_buff_msg = f"提升{int(atk_buff * 100)}%攻击力！" if atk_buff != 0 else ''
        crit_buff_msg = f"提升{int(crit_buff * 100)}%会心率！" if crit_buff != 0 else ''
...
def get_armor_info_msg(armor_id, armor_info=None):
    """
    获取一个防具信息msg，有 desc 时优先使用；否则由 def_buff/atk_buff/crit_buff 拼效果
    """
    msg = ''
    ...
    if armor_info.get('desc'):
        for line in armor_info['desc'].strip().split('\n'):
            if line.startswith('效果：'):
                msg += line.rstrip() + '\n'
                return msg
        msg += armor_info['desc'].strip() + '\n'
    else:
        def_buff = armor_info.get('def_buff', 0) or 0
        atk_buff = armor_info.get('atk_buff', 0) or 0
        crit_buff = armor_info.get('crit_buff', 0) or 0
        parts = []
        if def_buff != 0:
            parts.append(f"提升{int(def_buff * 100)}%减伤率！")
        if atk_buff != 0:
            parts.append(f"提升{int(atk_buff * 100)}%攻击力！")
        if crit_buff != 0:
            parts.append(f"提升{int(crit_buff * 100)}%会心率！")
        msg += f"效果：{''.join(parts)}"
    return msg

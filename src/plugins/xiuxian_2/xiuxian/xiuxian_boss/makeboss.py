...
def _safe_index(prefix: str) -> int:
    try:
        return jinjie_list.index(prefix)
    except ValueError:
        return 0


def createboss(max_level_prefix: str | None = None):
    if not max_level_prefix:
        top_user_info = sql_message.get_top1_user()
        top_user_level = top_user_info.level
        max_level_prefix = _level_prefix(top_user_level)
    idx = _safe_index(max_level_prefix)
    # 境界池：上限-3 到 上限+3（共最多7个境界）
    start = max(0, idx - 3)
    end = min(len(jinjie_list), idx + 4)
    pool = jinjie_list[start:end]
    boss_jj = random.choice(pool)

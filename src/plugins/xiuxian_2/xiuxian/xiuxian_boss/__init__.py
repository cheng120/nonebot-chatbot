...
from .makeboss import createboss, createboss_root, jinjie_list
...
async def _get_group_max_level_prefix(group_id: str) -> str | None:
	"""
	按“本群活跃人群”境界计算Boss境界上限（大境界）。
	活跃 = 最近 Boss活跃天数 内有签到的成员；只统计已注册修仙的成员。
	若无活跃成员，则退化为按全群已注册成员的最高境界；若全群无可用成员，返回 None。
	"""
	try:
		bot = get_bot()
	...
	active_days = int(config.get("Boss活跃天数", 7))
	now = int(time.time())
	...
	for m in members:
		try:
			...
			continue
		user = sql_message.get_user_message(uid)
		if user is None or not getattr(user, "level", None):
			continue
		prefix = _level_prefix(user.level)
		try:
			idx = jinjie_list.index(prefix)
		except ValueError:
			idx = 0
		if idx > max_idx_any:
			max_idx_any = idx
		st = getattr(user, "sign_in_time", None)
		if st is not None and (isinstance(st, (int, float)) and int(st) >= active_cutoff):
			if idx > max_idx_active:
				max_idx_active = idx

	# 优先用活跃成员最高境界；无活跃成员时用全群最高境界
	use_idx = max_idx_active if max_idx_active >= 0 else max_idx_any
	if use_idx < 0:
		return None
	return jinjie_list[use_idx]
...
async def send_bot(group_id:str):
        ...
        if len(group_boss[group_id]) >= config['Boss个数上限']:
            ...
            max_prefix = await _get_group_max_level_prefix(group_id)
            bossinfo = createboss(max_level_prefix=max_prefix)
            ...
            bot_id = layout_bot_dict.get(group_id) if layout_bot_dict else None
            if bot_id is None and put_bot:
                bot_id = put_bot[0]
            try:
                if bot_id is None:
                    await get_bot().call_api(api, **data)
                elif type(bot_id) is str:
                    await get_bots()[bot_id].call_api(api, **data)
                elif type(bot_id) is list and bot_id:
...
async def battle_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    user_buff_data = UserBuffDate(userinfo.user_id)
    ...
    crit = (int((user_weapon_data.get('crit_buff') or 0) * 100) if user_weapon_data else 0) + (int((user_armor_data.get('crit_buff') or 0) * 100) if user_armor_data else 0)
    player['会心'] = crit if crit > 0 else 1
    ...
    if victor == "Boss赢了":
        ...
        # 新增boss战斗积分点数
        boss_all_hp = bossinfo['总血量']  # 总血量
        boss_integral = int((boss_old_hp / boss_all_hp) * 30 * 10)  # 击杀世界boss积分*10
        if boss_integral < 5:  # 摸一下不给
            boss_integral = 0
        if user_info.root == "器师":
            boss_integral = int(boss_integral * (1 + (user_rank - boss_rank)))

...
UserDate = namedtuple("UserDate",
                      ["id", "user_id", "stone", "root", "root_type", "level", "power", "create_time", "is_sign", "exp",
                       ...
                       "atkpractice",
                       "sect_task", "sect_contribution", "sect_elixir_get", "blessed_spot_flag", "blessed_spot_name", "sign_in_time"])
...
class XiuxianDateManage:
  ...
  "fabao_weapon" integer DEFAULT 0
    ...
    def get_sign(self, user_id):
        """获取用户签到信息。奖励CD由 sign_in_reward_cd_hours 控制（小时）。"""
        cur = self.conn.cursor()
        try:
            sql = f"select is_sign, sign_in_time from user_xiuxian where user_id=?"
            cur.execute(sql, (user_id,))
            result = cur.fetchone()
        except sqlite3.OperationalError:
        ...
        is_sign = result[0]
        last_ts = int(result[1]) if len(result) > 1 and result[1] else 0
        now_ts = int(time.time())
        cd_seconds = XiuConfig().sign_in_reward_cd_hours * 3600
        if last_ts == 0:
            ls = random.randint(XiuConfig().sign_in_lingshi_lower_limit, XiuConfig().sign_in_lingshi_upper_limit)
            sql2 = f"UPDATE user_xiuxian SET is_sign=1,stone=stone+?,sign_in_time=? WHERE user_id=?"
            cur.execute(sql2, (ls, now_ts, user_id))
            self.conn.commit()
        ...
        if elapsed >= cd_seconds:
            ...
            self.conn.commit()
            return '签到成功，获取{}块灵石!'.format(ls)
        remain = cd_seconds - elapsed
        return '贪心的人是不会有好运的！还需{}小时{}分钟可签到'.format(remain // 3600, (remain % 3600) // 60)
    ...
    def get_user_buff_info(self, user_id):
        ...
        if not result:
            return None
        # 兼容旧表缺少 sub_buff 列：BuffInfo 需 10 个字段
        if len(result) == 9:
            result = result + (0,)
...
class XiuxianJsonDate:
    ...
    def linggen_get(self, chaos_rate_multiply=1):
        """获取灵根信息。chaos_rate_multiply: 超管重入仙途时混沌灵根概率倍数，默认1"""
        data = jsondata.root_data()
        rate_dict = {}
        for i, v in data.items():
            rate_dict[i] = v["type_rate"]
        if chaos_rate_multiply != 1 and "混沌灵根" in rate_dict:
            rate_dict["混沌灵根"] = int(rate_dict["混沌灵根"] * chaos_rate_multiply)
            if rate_dict["混沌灵根"] < 1:
                rate_dict["混沌灵根"] = 1
...
class OtherSet(XiuConfig):
    ...
    def player_fight(self, player1: dict, player2: dict):
        ...
        """
        from .player_fight import get_user_def_buff
        ...
        while True:
            ...
            if random.randint(0, 100) <= max(0, min(100, int(player1['会心']))):
                player1_gj = int(player1_gj * float(player1['爆伤']))
                msg1 = "{}发起会心一击，造成了{}伤害\n"

            player2_gj = int(round(random.uniform(0.95, 1.05), 2) * player2['攻击'])
            if random.randint(0, 100) <= max(0, min(100, int(player2['会心']))):
                player2_gj = int(player2_gj * float(player2['爆伤']))
                msg2 = "{}发起会心一击，造成了{}伤害\n"

            play1_sh: int = int(player1_gj * get_user_def_buff(player2['user_id']))
            play2_sh: int = int(player2_gj * get_user_def_buff(player1['user_id']))
...
def final_user_data(user_data):
    ...
    # 每级+4%攻击（hp=15, mp=16, atk=17, atkpractice=18；sign_in_time 在末尾）

...
class XiuConfig:
    def __init__(self):
        ...
        self.sql_user_xiuxian = ["id", "user_id", "stone", "root",
                                 ...
                                 "blessed_spot_name", "sign_in_time"]  # 数据库字段校验（sign_in_time 为 ALTER 追加，在表末尾）
        ...
        self.sql_buff = ["id", "user_id", "main_buff", "sec_buff", "faqi_buff", "fabao_weapon", "armor_buff",
                         "atk_buff", "blessed_spot", "sub_buff"]
        ...
        self.sign_in_reward_cd_hours = 6  # 签到奖励CD（小时），间隔内可再次领取奖励
        self.tou = 1000000  # 偷灵石惩罚
        self.tou_cd = 300  # 偷灵石cd/秒
        self.tou_lower_limit = 0.01  # 偷灵石下限(百分比)
        self.tou_upper_limit = 0.2  # 偷灵石上限(百分比)
        self.remake = 100000  # 重入仙途的消费(灵石)
        self.remake_cd = 5  # 重入仙途CD/秒
        self.del_boss_id = []  # 支持非管理员和超管天罚boss
        self.use_forward_msg = False  # 合并转发：NapCat 等不支持时设为 False，传承抽卡等直接发普通消息
...
class JsonConfig:
    ...
    def read_data(self):
        ...
        if not self.config_jsonpath.exists():
            ...
            logger.warning(f"修仙配置文件不存在: {self.config_jsonpath}，使用默认配置。")
            # 返回默认配置结构
            default_config = {
                "group": []  # 默认没有启用修仙功能的群
            }
            # 尝试创建目录和文件
            try:
                self.config_jsonpath.parent.mkdir(parents=True, exist_ok=True)
                with open(self.config_jsonpath, 'w', encoding='utf-8') as f:
                    json.dump(default_config, f, ensure_ascii=False, indent=4)
                logger.info(f"已创建默认配置文件: {self.config_jsonpath}")

...
def Cooldown(
    ...
    async def dependency(bot: Bot, matcher: Matcher, event: MessageEvent):
        ...
        if group_id not in conf_data["group"]:
            logger.debug(f"修仙插件: 群 {group_id} 未启用修仙功能，阻止命令处理")
            assign_bot_inst = await assign_bot_group(group_id=group_id)
            if (
                    event.sender.role == "admin" or
                    event.sender.role == "owner" or
                    event.get_user_id() in bot.config.superusers
            ):
                await assign_bot_inst.send(event=event, message=MessageSegment.at(event.get_user_id()) + f"本群已关闭修仙模组,请联系管理员开启,开启命令为【启用修仙功能】!")
                await matcher.finish()
            else:
                await assign_bot_inst.send(event=event, message="本群未开启修仙功能，请联系管理员使用「启用修仙功能」开启。")
                await matcher.finish()
        else:
            pass
        if running[key] <= 0:

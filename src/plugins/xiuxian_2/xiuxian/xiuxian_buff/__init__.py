...
from ..player_fight import Player_fight, get_wearer_fight_modifiers, get_user_def_buff
from ..utils import (
...
async def qc_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if give_qq:
        ...
        user1_buff = UserBuffDate(user_id)
        user1_weapon_data = user1_buff.get_user_weapon_data()
        user1_armor_data = user1_buff.get_user_armor_buff_data()
        crit1 = (int((user1_weapon_data.get('crit_buff') or 0) * 100) if user1_weapon_data else 0) + (int((user1_armor_data.get('crit_buff') or 0) * 100) if user1_armor_data else 0)
        player1['会心'] = crit1 if crit1 > 0 else 1

        user2 = sql_message.get_user_real_info(give_qq)
        user2_buff = UserBuffDate(user2.user_id)
        user2_weapon_data = user2_buff.get_user_weapon_data()
        user2_armor_data = user2_buff.get_user_armor_buff_data()
        crit2 = (int((user2_weapon_data.get('crit_buff') or 0) * 100) if user2_weapon_data else 0) + (int((user2_armor_data.get('crit_buff') or 0) * 100) if user2_armor_data else 0)
        player2['会心'] = crit2 if crit2 > 0 else 1
...
async def out_closing_(bot: Bot, event: GroupMessageEvent):
    ...
    if not is_type:
        ...
        exp_time = exp_time_raw * 10  # 闭关回复速度 10 倍：相同现实时间按 10 倍修炼时长结算
        ...
        if exp >= user_get_exp_max:
            ...
            if str(event.message) == "灵石出关":
                ...
                if exp <= user_stone:
                    ...
                    msg = "闭关结束，共闭关{}分钟，本次闭关增加修为：{}，消耗灵石{}枚{}{}".format(exp_time_raw, exp, int(exp / 2),
                    ...
                    sql_message.update_user_attribute(user_id, result_hp_mp[0], result_hp_mp[1],
                                                      int(result_hp_mp[2] / 10))
                    msg = "闭关结束，共闭关{}分钟，本次闭关增加修为：{}，消耗灵石{}枚{}{}".format(exp_time_raw, exp, user_stone,
                                                                        result_msg[0], result_msg[1])
                ...
                result_msg, result_hp_mp = OtherSet().send_hp_mp(user_id, int(exp * hp_speed), int(exp * mp_speed))
                sql_message.update_user_attribute(user_id, result_hp_mp[0], result_hp_mp[1], int(result_hp_mp[2] / 10))
                msg = "闭关结束，共闭关{}分钟，本次闭关增加修为：{}{}{}".format(exp_time_raw, exp, result_msg[0],
                                                            result_msg[1])
                if XiuConfig().img:
                    pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
...
async def mind_state_(bot: Bot, event: GroupMessageEvent):
    ...
    if user_weapon_data is not None:
        crit_buff += int((user_weapon_data.get('crit_buff') or 0) * 100)
    user_armor_data = user_buff_data.get_user_armor_buff_data()
    if user_armor_data is not None:
        crit_buff += int((user_armor_data.get('crit_buff') or 0) * 100)
    if crit_buff == 0:
        crit_buff = 1

    # 减伤率：展示最终有效减伤（含装备上的增减伤修正）
    def_buff = int((1 - get_user_def_buff(user_id)) * 100)
    ...
    cd, _, _, cda, cdm = get_wearer_fight_modifiers(user_id)
    crit_total = max(0, min(100, crit_buff + int(impart_know_per * 100) + cd))
    crit_dmg_total = max(0.12, (1.5 + impart_burst_per + cda) * cdm)

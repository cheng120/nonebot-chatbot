...
def _get_sect_exp(level: int) -> str:
    """安全获取宗门经验配置，如果文件不存在返回默认值"""
    try:
        sect_data = jsondata.sect_config_data()
        if sect_data and str(level) in sect_data:
            return str(sect_data[str(level)].get("max_exp", "未知"))
        return "未知"
    except Exception:
        return "未知"
...
{_get_sect_exp(0)}  {_get_sect_exp(1)}  {_get_sect_exp(2)}
{_get_sect_exp(3)}  {_get_sect_exp(4)}  {_get_sect_exp(4)}
""".strip()
    ...
    if sect_id:
        ...
        if sect_position == 4:
            ...
            if int(sect_info.elixir_room_level) == 1:
                due_num = 1 + extra_due
                msg = f"道友成功领取到丹药：渡厄丹 {due_num} 枚！"
                sql_message.send_back(user_info.user_id, 1999, "渡厄丹", "丹药", due_num, bind_flag=1)
            ...
            if sect_info.sect_used_stone >= stonecost and sect_info.sect_materials >= materialscost:
                if random.randint(0, 100) <= mainbuffconfig['获取到功法的概率']:
                    ...
                    if mainbuffid in mainbuffidlist:
                        ...
                        await sect_mainbuff_get.finish()
                    sql_message.update_sect_materials(sect_id, materialscost, 2)
                    sql_message.update_sect_scale_and_used_stone(sect_id, sect_info.sect_used_stone - stonecost,
...
async def upatkpractice_(bot: Bot, event: GroupMessageEvent):
    ...
    if sect_id:
        ...
        sect_level = get_sect_level(sect_id)[0] if get_sect_level(sect_id)[
                                                       0] <= 25 else 25  # 获取当前宗门修炼等级上限，500w建设度1级,上限25级
...
@sect_task_refresh.handle(parameterless=[Cooldown(cd_time=config['宗门任务刷新cd'], at_sender=True)])
...
async def create_sect_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if (list_level_all.index(level) < list_level_all.index(XiuConfig().sect_min_level) or
...
def get_sectbufftxt(sect_scale, config_):
    ...
    bufftxt = {1: '人阶下品', 2: '人阶上品', 3: '黄阶下品', 4: '黄阶上品', 5: '玄阶下品', 6: '玄阶上品', 7: '地阶下品', 8: '地阶上品', 9: '天阶下品',
               10: '天阶上品', 11: '仙阶下品', 12: '仙阶上品', 13: '仙阶极品', 14: '无上仙法'}

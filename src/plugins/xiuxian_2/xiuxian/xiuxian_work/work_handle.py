...
class workhandle(XiuxianJsonDate):
    ...
    def do_work(self, key, work_list=None, name=None, level="江湖好手", exp=None, user_id=None, accept_count=0, is_superuser=False):
        """悬赏令获取。accept_count：当日接取次数，任务时长递增 次数*2 分钟。is_superuser：超管时装备概率5%，否则1%。"""
        if key == 0:  # 如果没有获取过，则返回悬赏令
            data = workmake(level, exp, XiuxianDateManage().get_user_message(user_id).level, accept_count=accept_count, is_superuser=is_superuser)
            get_work_list = []

...
work_accept_count: Dict[str, int] = {}  # 用户当日接取次数，任务时长递增=次数*2分钟
...
async def resetrefreshnum_():
    global refreshnum, work_accept_count
    refreshnum = {}
    work_accept_count = {}
    logger.info("用户悬赏令刷新次数、接取次数已重置")
...
async def last_work_(bot: Bot, event: GroupMessageEvent):
    ...
    if is_type:
        ...
        if exp_time < time2:
            ...
            reward_mult = 3 if USERRANK[user_info.level] <= 23 else 1  # 虚道境圆满以上奖励3倍
            give_stone = int(give_stone * reward_mult)
            item_flag = False
            item_msg = None
            item_info = None
            if item_id != 0:
...
async def do_work_(bot: Bot, event: GroupMessageEvent, args: Tuple[Any, ...] = RegexGroup()):
    ...
    if user_cd_message is None:
        await bot.send_group_msg(group_id=int(send_group_id), message="状态数据异常，请稍后再试")
        await do_work.finish()
    ...
    if mode == "刷新":  # 刷新逻辑
        ...
        accept_count = work_accept_count.get(str(user_id), 0)
        is_superuser = str(event.get_user_id()) in getattr(bot.config, "superusers", [])
        work_msg = workhandle().do_work(0, level=user_level, exp=user_info.exp, user_id=user_id, accept_count=accept_count, is_superuser=is_superuser)
        n = 1
        work_list = []
        ...
        if is_type:
            ...
            if exp_time < time2:
                ...
                msg, give_exp, s_o_f, item_id, big_suc = workhandle().do_work(2,
                                                                              ...
                                                                              user_id=user_info.user_id)
                reward_mult = 3 if USERRANK[user_info.level] <= 23 else 1  # 虚道境圆满以上奖励3倍
                give_exp = int(give_exp * reward_mult)
                item_flag = False
                item_info = None
                item_msg = None
                if item_id != 0:
        ...
        if is_type:  # 接取逻辑
            ...
            try:
                ...
                try:
                    ...
                    del work[user_id]
                    work_accept_count[str(user_id)] = work_accept_count.get(str(user_id), 0) + 1

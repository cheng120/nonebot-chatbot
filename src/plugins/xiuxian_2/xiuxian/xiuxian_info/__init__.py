...
xiuxian_info_other = on_command("修仙信息", aliases={"查修仙信息"}, priority=22, permission=GROUP, block=True)
sql_message = XiuxianDateManage()  # sql类


async def _send_user_info(bot: Bot, send_group_id: int, user_id: int) -> bool:
    """根据 user_id 拉取并发送该用户的修仙信息（图文或文字）。用户不存在返回 False。"""
...
async def xiuxian_message_(bot: Bot, event: GroupMessageEvent):
    ...
    await _send_user_info(bot, int(send_group_id), user_id)
    await xiuxian_message.finish()


@xiuxian_info_other.handle(parameterless=[Cooldown(cd_time=XiuConfig().user_info_cd, at_sender=True)])
async def xiuxian_info_other_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    """修仙信息：查看他人，用法 修仙信息 @某人 或 修仙信息 道号"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    target_id = None
    for seg in event.message:
        if seg.type == "at":
            target_id = int(seg.data.get("qq", 0)) or None
            break
    if target_id is None:
        name = args.extract_plain_text().strip()
        if name:
            target_user = sql_message.get_user_message2(name)
            if target_user:
                target_id = target_user.user_id
    if target_id is None:
        msg = "请@某人或输入道号查看其修仙信息，如：修仙信息 @xxx 或 修仙信息 大幸运"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
            await bot.send_group_msg(group_id=int(send_group_id), message=msg)
        await xiuxian_info_other.finish()
    ok = await _send_user_info(bot, int(send_group_id), target_id)
    if not ok:
        msg = "对方未踏入修仙界。"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)

...
async def Boss_fight(player1: dict, boss: dict, type_in=2, bot_id=0):
    ...
    while True:
        ...
        if player1['气血'] <= 0:  # 玩家2气血小于0，结算
            ...
            suc = "Boss赢了"
            # 防止因计算异常导致 sh<0，从而出现“扣灵石”的情况
            get_stone = max(0, int(boss_now_stone * (sh / qx)))
...
def get_wearer_fight_modifiers(user_id):
	"""法器/防具上扩展战斗属性（与 desc 一致）"""
	wd = UserBuffDate(user_id).get_user_weapon_data()
	ad = UserBuffDate(user_id).get_user_armor_buff_data()
	crit_delta_pct = 0
	armor_def_penalty = 0.0
	armor_def_bonus = 0.0
	crit_dmg_add = 0.0
	crit_dmg_mult = 1.0
	for eq in (wd, ad):
		if not eq:
			continue
		crit_delta_pct += int(eq.get("wearer_crit_delta_pct") or 0)
		armor_def_penalty += float(eq.get("wearer_armor_def_penalty") or 0)
		armor_def_bonus += float(eq.get("wearer_armor_def_bonus") or 0)
		crit_dmg_add += float(eq.get("wearer_crit_damage_add") or 0)
		m = eq.get("wearer_crit_damage_mult")
		if m is not None:
			crit_dmg_mult *= float(m)
	return crit_delta_pct, armor_def_penalty, armor_def_bonus, crit_dmg_add, crit_dmg_mult


def get_user_def_buff(user_id):
	user_armor_data = UserBuffDate(user_id).get_user_armor_buff_data()
	def_buff = float(user_armor_data["def_buff"]) if user_armor_data is not None else 0.0
	_, pen, bonus, _, _ = get_wearer_fight_modifiers(user_id)
	def_buff = def_buff + bonus - pen
	def_buff = max(0.0, min(0.92, def_buff))
	return round(1.0 - def_buff, 2)
...
def get_turnatk(player, buff=0):
	...
	cd, _, _, cda, cdm = get_wearer_fight_modifiers(user_id) if user_id else (0, 0.0, 0.0, 0.0, 1.0)
	isCrit = False
	turnatk = int(round(random.uniform(0.95, 1.05), 2) * (player["攻击"] * (buff + 1)))
	crit_rate = int(player.get("会心", 0)) + int(impart_know_per * 100) + cd
	crit_rate = max(0, min(100, crit_rate))
	if random.randint(0, 100) <= crit_rate:
		mult = (1.5 + impart_burst_per + cda) * cdm
		mult = max(0.12, mult)
		turnatk = int(turnatk * mult)
		isCrit = True

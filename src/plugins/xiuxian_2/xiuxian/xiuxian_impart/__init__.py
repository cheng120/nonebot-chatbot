        ...
        if get_rank(user_id):
            ...
            if impart_data_json.data_person_add(user_id, reap_img):
                ...
                list_tp.append(
                    {"type": "node", "data": {"name": f"道友{user_info.user_name}的传承抽卡", "uin": bot.self_id,
                                                  "content": img}})
                if XiuConfig().use_forward_msg:
                    try:
                        await send_forward_msg_list(bot, event, list_tp)
                    except Exception as e:
                        logger.warning(f"传承抽卡合并转发失败，改用普通消息: {e!r}")
                        await bot.send_group_msg(group_id=int(send_group_id), message=msg)
                        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(img_path_ya / str(reap_img + ".png")))
                else:
                    await bot.send_group_msg(group_id=int(send_group_id), message=msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(img_path_ya / str(reap_img + ".png")))
            ...
            if XiuConfig().use_forward_msg:
                try:
                    ...
                    await bot.send_group_msg(group_id=int(send_group_id), message=msg)
                    for x in time_img[:3]:
                        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(img_path_ya / str(x + ".png")))
            else:
                await bot.send_group_msg(group_id=int(send_group_id), message=msg)
                for x in time_img[:3]:
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(img_path_ya / str(x + ".png")))

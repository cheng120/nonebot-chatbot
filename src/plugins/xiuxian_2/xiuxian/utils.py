...
# wcwidth 未使用，已移除导入以避免依赖问题
# 如果需要计算字符宽度，可以使用 unicodedata.east_asian_width() 或 PIL 的 getlength()
import os
...
async def send_forward_msg_list(
    ...
    """
    合并消息转发（群/私聊）。
    常见失败原因：1) NapCat 不支持直接构造合并转发，仅支持按消息 id 转发；
    2) 部分实现要求 node.data 含 user_id/nickname 或 content 为消息数组；
    3) 实现未支持 send_group_forward_msg。失败时调用方应 fallback 为普通消息。
    """

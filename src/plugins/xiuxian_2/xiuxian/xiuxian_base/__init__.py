...
from ..read_buff import UserBuffDate, items as xiuxian_items
...
from ..player_fight import get_wearer_fight_modifiers
...
class _GroupAdminOrOwner:
	...
	async def __call__(self, bot: Bot, event: Event) -> bool:
		...
		if role in ("admin", "owner"):
			return True
		# 有些实现不会在 sender 内带 role，这里兜底拉取一次群成员信息再判定
		try:
			...
			return (info or {}).get("role") in ("admin", "owner")
		except Exception:
			return False


# 启用/禁用修仙：超管 或 群主/管理员
OPEN_XIUXIAN_PERMISSION = SUPERUSER | Permission(_GroupAdminOrOwner())
...
level_order = on_command("查看境界顺序", aliases={"境界顺序", "境界列表"}, priority=7, permission=GROUP, block=True)
equipment_level_query = on_command("装备等级查询", aliases={"查询装备等级", "装备品阶"}, priority=7, permission=GROUP, block=True)
...
open_xiuxian = on_command("启用修仙功能", aliases={'禁用修仙功能'}, permission=OPEN_XIUXIAN_PERMISSION, priority=5, block=True)
...
• 我要修仙 / 加入修仙 - 加入修仙系统
• 修仙签到 / 签到 - 每日签到获取灵石及修为
• 重入仙途 - 重置灵根数据(每次{XiuConfig().remake}灵石)
• 改名 - 修改你的道号
• 突破 / 渡厄突破 / 直接突破 - 境界突破
• 我的修仙信息 / 我的存档 - 查看完整修仙数据
• 修仙排行榜 / 排行榜 - 查看各类排行榜
...
【世界Boss】
• 生成世界boss - 生成世界Boss(超管权限)
• 查询世界boss - 查询世界Boss信息
• 世界boss - 设置群Boss(管理员权限)
• 讨伐boss - 讨伐世界Boss
• 天罚boss - 天罚Boss(超管权限)
• 世界积分查看 / 查看世界积分 - 查看世界积分
• 世界积分兑换 - 使用世界积分兑换物品
• 世界boss帮助 - 查看Boss帮助
...
【其他功能】
• 送灵石 - 赠送灵石(格式:送灵石100@xxx)
• 偷灵石 / 飞龙探云手 - 偷取灵石(格式:偷灵石@xxx)
• 抢劫 / 抢灵石 - 抢劫灵石(格式:抢劫@xxx)
• 修仙适配 - 将1的境界适配到2
• 启用修仙功能 / 禁用修仙功能 - 开启或关闭修仙功能(超管权限)
• 更新记录 - 获取插件最新内容
• 风控帮助 - 查看风控配置帮助

═══════════════════════════════════
提示: 各模块详细帮助请发送对应模块的帮助命令
═══════════════════════════════════
""".strip()
...
async def run_xiuxian_(bot: Bot, event: GroupMessageEvent):
    """加入修仙"""
    logger.info(f"修仙插件: 收到'我要修仙'命令，用户{event.user_id}，群{event.group_id}")
    bot, send_group_id = await assign_bot(bot=bot, event=event)
...
@restart.handle(parameterless=[Cooldown(XiuConfig().remake_cd, at_sender=True)])
async def restart_(bot: Bot, event: GroupMessageEvent):
    ...
    user_id = user_info.user_id
    is_superuser = str(event.get_user_id()) in bot.config.superusers
    name, root_type = XiuxianJsonDate().linggen_get(chaos_rate_multiply=10 if is_superuser else 1)
...
@level_order.handle()
async def level_order_(bot: Bot, event: GroupMessageEvent):
    """查看境界顺序（从低到高）"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    level_data = jsondata.level_data()
    if not level_data:
        msg = "境界数据为空，请检查 data/xiuxian/境界.json"
    else:
        msg = "✨境界顺序（从低到高）✨\n"
        for idx, name in enumerate(level_data.keys(), 1):
            msg += f"{idx}. {name}\n"
    if XiuConfig().img:
        pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        ...
        await bot.send_group_msg(group_id=int(send_group_id), message=msg)
    await level_order.finish()


@equipment_level_query.handle()
async def equipment_level_query_(bot: Bot, event: GroupMessageEvent):
    """装备等级（品阶）查询：列出法器与防具按品阶分类"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    try:
        equipments = xiuxian_items.get_data_by_item_type(["法器", "防具"])
    except Exception:
        equipments = {}
    if not equipments:
        msg = "装备数据为空，请检查 data/xiuxian/装备 下法器.json、防具.json"
    else:
        by_level = {}
        for eid, info in equipments.items():
            lv = info.get("level", info.get("rank", "?"))
            rank_num = info.get("rank", lv)
            try:
                sort_key = (int(rank_num), str(lv))
            except (TypeError, ValueError):
                sort_key = (999, str(lv))
            if sort_key not in by_level:
                by_level[sort_key] = []
            by_level[sort_key].append((info.get("item_type", "装备"), info.get("name", "?"), eid))
        msg = "✨装备品阶列表✨\n"
        for (_, lv_str), eq_list in sorted(by_level.items(), key=lambda x: x[0]):
            parts = [f"{t}-{name}({eid})" for t, name, eid in eq_list]
            msg += f"品阶{lv_str}: " + "；".join(parts) + "\n"
    if XiuConfig().img:
        pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
    else:
        await bot.send_group_msg(group_id=int(send_group_id), message=msg)
    await equipment_level_query.finish()
...
async def level_up_zj_(bot: Bot, event: GroupMessageEvent):
    ...
    level_rate = jsondata.level_rate_data()[level_name]  # 对应境界突破的概率
    user_leveluprate = int(user_msg.level_up_rate)  # 用户失败次数加成
    break_rate = level_rate + user_leveluprate
    if str(event.get_user_id()) in bot.config.superusers:
        break_rate += 10  # 超管突破概率+10%
    le = OtherSet().get_type(exp, break_rate, level_name)
    if le == "失败":
        # 突破失败
...
async def rob_stone_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if give_qq:
        ...
        if user_2:
            ...
            if int(user_2.hp) <= int(user_2.exp) / 10:
            ...
            if int(user_info.hp) <= int(user_info.exp) / 10:
            ...
            user1_weapon_data = user_buff_data.get_user_weapon_data()
            user_armor_data = user_buff_data.get_user_armor_buff_data()
            crit1 = (int((user1_weapon_data.get('crit_buff') or 0) * 100) if user1_weapon_data else 0) + (int((user_armor_data.get('crit_buff') or 0) * 100) if user_armor_data else 0) + int((0.01 + (impart_data_1.impart_know_per if impart_data_1 is not None else 0)) * 100)
            cd1, _, _, cda1, cdm1 = get_wearer_fight_modifiers(user_id)
            player1['会心'] = max(0, min(100, crit1 + cd1))
            impart_b1 = impart_data_1.impart_burst_per if impart_data_1 is not None else 0
            player1['爆伤'] = max(0.12, (1.5 + impart_b1 + cda1) * cdm1)
            ...
            user2_weapon_data = user_buff_data.get_user_weapon_data()
            user_armor_data = user_buff_data.get_user_armor_buff_data()
            crit2 = (int((user2_weapon_data.get('crit_buff') or 0) * 100) if user2_weapon_data else 0) + (int((user_armor_data.get('crit_buff') or 0) * 100) if user_armor_data else 0) + int((0.01 + (impart_data_2.impart_know_per if impart_data_2 is not None else 0)) * 100)
            cd2, _, _, cda2, cdm2 = get_wearer_fight_modifiers(user_2.user_id)
            player2['会心'] = max(0, min(100, crit2 + cd2))
            impart_b2 = impart_data_2.impart_burst_per if impart_data_2 is not None else 0
            player2['爆伤'] = max(0.12, (1.5 + impart_b2 + cda2) * cdm2)

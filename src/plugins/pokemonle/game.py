...
class Pokemonle:
    def __init__(self):
        ...
        logger.info(f"[pokemonle] 配置读取: max_attempts={plugin_config.pokemonle_max_attempts}, gens={plugin_config.pokemonle_gens}, cheat={plugin_config.pokemonle_cheat}")
        ...
        logger.info(f"[pokemonle] 加载了 {len(self.pokeList)} 个宝可梦（世代过滤后）")
    ...
    def _load_and_filter_poke(self):
        ...
        # 将配置中的整数世代转换为字符串格式（如 1 -> "第一世代"）
        from .utils.pokeUtils import Gens
        target_gens = []
        for gen in plugin_config.pokemonle_gens:
            if isinstance(gen, int) and 1 <= gen <= len(Gens):
                # 整数格式：1-9 转换为 "第一世代" - "第九世代"
                target_gens.append(Gens[gen - 1])
            elif isinstance(gen, str):
                # 字符串格式：直接使用
                target_gens.append(gen)
        
        if not target_gens:
            # 如果转换后为空，返回所有宝可梦
            from loguru import logger
            logger.warning(f"[pokemonle] 世代配置无效: {plugin_config.pokemonle_gens}，将使用所有世代")
            return newPokeList
        
        filtered = [p for p in newPokeList if p.get('generation') in target_gens]
        from loguru import logger
        logger.info(f"[pokemonle] 世代过滤: 配置={plugin_config.pokemonle_gens}, 转换后={target_gens}, 原始数量={len(newPokeList)}, 过滤后数量={len(filtered)}")
        return filtered
    ...
    def start_new_game(self, uninfo: Uninfo) -> Dict:
        ...
        # 检查宝可梦列表是否为空
        if not self.pokeList:
            from loguru import logger
            logger.error(f"[pokemonle] 宝可梦列表为空，无法开始游戏。配置的世代: {plugin_config.pokemonle_gens}")
            raise ValueError("宝可梦列表为空，请检查世代配置是否正确")

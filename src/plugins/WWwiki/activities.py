...
# 鸣潮卡池列表命令
pool_list_spath = get_template("pool")
pool_list = on_command('鸣潮卡池列表', aliases={'卡池列表', '鸣潮卡池'})


@pool_list.handle()
async def pool_list_handler():
    async with httpx.AsyncClient() as client:
        resp = await client.post(url, data=data, headers=headers)
    acdata = resp.json()
    pool_content = await pool(acdata)
    ...
    img = await draw_main(
        Data,
        pool_list_spath.name,
        pool_list_spath.parent.as_posix(),
    )

...
# 配置目录：优先使用项目内 config/suggarchat，保证与 UniConfigManager 读取同一 config.toml
_cwd_suggarchat = Path.cwd() / "config" / "suggarchat"
if _cwd_suggarchat.is_dir():
	CONFIG_DIR = _cwd_suggarchat
else:
	try:
		CONFIG_DIR = store.get_plugin_config_dir()
	except RuntimeError:
		CONFIG_DIR = store.get_config_dir("suggarchat")
driver = get_driver()
...
class ConfigManager(EnvfulConfigManager[Config]):
    config_class = Config
    # 使用固定名 "suggarchat" 使 UniConfigManager 读取 get_config_dir("suggarchat")，与 CONFIG_DIR 一致
    _owner_name: str = "suggarchat"
    ...
    async def load(self):
        ...
        # 若配置中为空，则从环境变量 OPENAI_* 填充 default_preset（与 .env 中的 LongCat 等配置一致）
        dp = self.ins_config.default_preset
        if not dp.model and os.getenv("OPENAI_DEFAULT_MODEL"):
            dp.model = os.getenv("OPENAI_DEFAULT_MODEL", "")
        if not dp.base_url and os.getenv("OPENAI_BASE_URL"):
            dp.base_url = os.getenv("OPENAI_BASE_URL", "")
        if not dp.api_key and os.getenv("OPENAI_API_KEY"):
            dp.api_key = os.getenv("OPENAI_API_KEY", "")

...
from .utils.models import TextContent
...
def _is_chat_command(message_text: str) -> bool:
	"""消息是否为 /chat 或 /chat xxx 形式（会先剥掉 command_start）"""
	prefixes = get_driver().config.command_start or ["/"]
	for p in prefixes:
		if p and message_text.startswith(p):
			rest = message_text[len(p):].strip()
			return rest == "chat" or rest.startswith("chat ")
	return False
...
async def should_respond_to_message(event: MessageEvent, bot: Bot) -> bool:
    ...
    if _is_chat_command(message_text):
        return True
